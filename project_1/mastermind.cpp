...
// Viraj Lawande
//
//Mastermind class implementation file that handles playing of the game, with functions to print the secret code for game testing,
//get the user's guess, get the number of correct and incorrect digits
//and check if the code has been cracked
...
// mastermind class constructor that takes in user inputs for the length and digit range for the code
{
...
// mastermind class constructor that takes in no inputs and initializes the secret code with a 5 digit length and a digit range of 10
{
    secretCode.initializeRandomCode();
    guessCounter = 0;
};
...
// function to get the response (number of correct and incorrect digits) of the code
{
...
// checks if the user has guessed the code by comparing the length of the secret code to the number of correct digits in the user's guess
{
