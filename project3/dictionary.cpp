...
// constructor for the dictionary class. Takes in the user input to determine
// which sorting algorithm should be used reads words from the dictionary file
// into the words vector, and then sorts those words using the selected sorting
// algorithm
{
  readWords();
  if (sortType == 0) {
...
// function to read the words in from the dictionary text file
{
                        ...
                        // reading in words
  {
    ...
    while (getline(
        myFile,
        line)) // pushes each line from the txt file as a string to the words
               // vector until the last line of the txt document is reached
    {
...
// puts the pivot at the correct position in a sorted array, and returns the
// index of the new pivot.
{
  string x = words[r];
  int i = p - 1;
