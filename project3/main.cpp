...
// and creates all possible words from the grid. it then searches for them in the dictionary
// and prints the matches to the screen
{
...
//search function that takes in an integer to determine which sorting algorithm the user would like to use,
//creates a dictionary sorted using the appropriate method, creates a character matrix from the filename that the user inputs
// and then calls the findMatches() function to find the words from the grid that match the ones in the dictionary
{
...
int main() 
//main function that prompts user to select which sorting algorithm they'd like to use, and then calls the search function
{
	int sortType;
