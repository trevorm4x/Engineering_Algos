...
//once all boards are solved, it also prints out the average number of recursive calls to solve a puzzle
{
    string response;
...
// readBigStringValues function that takes in a user inputted string and reads in the
...
//prints out the row, column and square conflicts
{
    for (int row = 0; row < 9; row++) 
    {
        for (int col = 0; col < 9; col++) 
        {
            ...
            {
                cout << (conflictType == 0   ? "row" : conflictType == 1 ? "col": "sqr") << " conflicts: {";
                for (int i = 0; i < 9; i++) {
...
bool board::isSolved() 
//boolean function that returns true if every cell on the board has been filled out
{
    for (int row = 0; row < 9; row++)
    {
        for (int col = 0; col < 9; col++)
...
//once the board is solved, it returns the number of recursive calls made in the function
{
    if (isSolved()) {
        return recurCalls;
    }
        ...
        for (int j = 0; j < 9; ++j) 
        {
            ...
            if (getCell(i, j) == -1) 
            {
                for (int num = 0; num < 9; ++num) 
                {
                    ...
                    if (conflicts[i][j][0][num] == 0 && conflicts[i][j][1][num] == 0 && conflicts[i][j][2][num] == 0) {
                        best.num[num] = 1;
                    }
                    else {
                        best.num[num] = 0;
